...
A verbatim skill body (overrides the placeholder transform, since the work
description comes from context and the scale reads awkwardly when substituted
inline).

```
Assess the complexity and risk of the work the user wants estimated — a task,
spec, or description from this conversation or a file they reference.

Use the estimation scale the user chose (default: T-shirt sizes — S/M/L/XL). If
the user gave relevant facts about the codebase (stack, existing patterns, known
debt), factor them in.

Produce a structured assessment:

## Complexity estimate
**Estimate:** [your estimate on the chosen scale]
**Confidence:** [high / medium / low]

**Reasoning:**
[2–4 sentences explaining the key factors driving the estimate. Be specific about
what makes this work more or less complex.]
...
## Assumptions
[Bullet list of assumptions made in producing the estimate. These are the
conditions that must be true for the estimate to hold. If any assumption is
wrong, the estimate changes.]
...
## Flags
[Any specific concerns that should be raised with a human before proceeding.
Write "None" if the work can proceed without escalation.]
```
