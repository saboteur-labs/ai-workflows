...
| [`agent-orchestration/retrospect-session.md`](./agent-orchestration/retrospect-session.md)       | low    | Turn a session's corrections into candidate rules, routed to the artifact that should carry each; checks recurrence across every project's log and prints the entry it appends |
