...
| [`agent-orchestration/retrospect-session.md`](./agent-orchestration/retrospect-session.md)       | low    | Turn a session's corrections into candidate rules, routed to a hook, an instruction file, a prompt, or an agent; checks recurrence across every project's log and prints the entry it appends |
