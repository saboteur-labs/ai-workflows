...
| [`agent-orchestration/retrospect-session.md`](./agent-orchestration/retrospect-session.md)       | low    | Turn a session's corrections into candidate rules, routed to a hook, an instruction file, a prompt, or an agent; checks recurrence across every project's log, records each rule's state and ref so "did it land" is answerable, and prints the entry it appends |
